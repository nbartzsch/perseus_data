...
---
title: "PERSEUS Medusa 02"
subtitle: "Fifth Firing - Detonation Setpoint with Aerospike"
date: 02.12.2024
author:
  - name: PERSEUS
    affiliation: 
...
#############################################################################
# Import data
processed_data_sens_1 <- read_csv(here("data", "processed_data/Medusa_02/detonation/firing_05", "filtered_signal_2.csv"))
processed_data_sens <- read_csv(here("data", "processed_data/Medusa_02/detonation/firing_05", "filtered_signal_1.csv"))
...
The Figures below show the unfiltered sensor response of both sensors. The sensors are offset by 90° in the combustion chamber and at different distances from the injector plate. The data below was gathered by Kistler sensors of type 601CAA placed in a recessed mounting chamber in the combustion chamber. A protective paste has been was applied to protect the sensors from thermal shock. In this firing the sensors could not withstand the heat and pressure even when mounted in a recessed chamber.

![](png_plots/Medusa_02/detonation/firing_05/response_time_01.svg){fig-align="center" width="500"}

![](png_plots/Medusa_02/detonation/firing_05/response_time_02.svg){fig-align="center" width="500"}
...
![](png_plots/Medusa_02/detonation/firing_05/stft_01.svg){fig-align="center" width="500"}

![](png_plots/Medusa_02/detonation/firing_05/stft_02.svg){fig-align="center" width="500"}

The original signal has been filtered with a High-Pass 4th order Butterworth filter with corner frequency at 1 kHz and a Low-Pass Butterworth filter with corner frequency at 65 kHz. The filtering was done to remove sensor drift, disturbances, Helmholtz Chamber resonance, high frequency noise etc.

### Sensor Response

@fig-resp-1 shows a section of the filtered signal over 1 ms. The responses of both sensors are shown in the plot.

```{r}
#| label: fig-resp-1
#| fig-cap: "Segment of sensor signal during 1 ms whit some phase bewtween both signals"
lower_time <- 4 # 4.2535, 4.3678, 4.3126, 4.00469, 4.26432
...
```{r}
#| label: fig-resp-2
#| fig-cap: "Segment of sensor signal during 1 ms with less phase shift"
lower_time <- 3.93525 #83019 
upper_time <- lower_time + 0.001
